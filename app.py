...
def download_dan_muat_model():
    # Menggunakan ID Drive kamu yang benar
...
st.markdown("""
    ...
    /* FIX: PAKSA TEMA GELAP AGAR TEKS PUTIH TERBACA DI HP */
...
with kanan:
    ...
    if foto and 'tombol' in locals() and tombol:
        with st.spinner('AI sedang memikir...'):
            ...
            # PENTING: Dibagi 255.0 agar AI tidak bingung
            arr = tf.keras.utils.img_to_array(img_res) / 255.0 
            arr = np.expand_dims(arr, 0)
            
            pred = model.predict(arr, verbose=0)
            
            # LOGIKA FIX: > 0.5 ADALAH ORGANIK (Berdasarkan tes Apel)
            if pred[0][0] > 0.5:
